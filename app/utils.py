...
def log_activity(user_id: int, action: str, resource_type: str, resource_id: Optional[int] = None, details: Optional[str] = None):
    ...
    try:
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        params = (user_id, action, resource_type, resource_id, details, datetime.now(timezone.utc))
...
def format_error_response(message: str, status_code: int = 400) -> tuple[Dict[str, str], int]:
    """Format error response consistently."""
    return {'error': message}, status_code


def format_success_response(data: Any = None, message: str = None, status_code: int = 200) -> tuple[Dict[str, Any], int]:
